...
def compute_similarity_matrix(seqs):
    """Compute a lower triangular pairwise similarity matrix using Numba for speed."""
    n = len(seqs)
    matrix = np.zeros((n, n), dtype=np.float32)

    for i in prange(n):  # Parallelize across rows
        for j in range(i + 1):
            ...
            for a, b in zip(seq1, seq2):
                if a != "-" and b != "-":  # Ignore gaps
                    ...
                    if a == b:
                        matches += 1
            matrix[i, j] = matrix[j, i] = matches / valid_positions if valid_positions > 0 else 0.0

    return matrix

def run_soverlap(family, folder, similarity_threshold, efficiency_factor):
    """
    ...
    # Estimate processing time (single-core baseline) cpu_count = 6
    time_in_minutes = round(0.000062 * (n ** 1.945) / 60, 2)
    # Adjust estimated time based on actual utilization
    adjusted_time_in_minutes = time_in_minutes / efficiency_factor
    # Convert to h:min:sec format
    hours = int(adjusted_time_in_minutes // 60)
    minutes = int(adjusted_time_in_minutes % 60)
    seconds = int((adjusted_time_in_minutes * 60) % 60)
    time_estimate = f"{hours}:{minutes:02d}:{seconds:02d}"
    finishing_time = datetime.now() + timedelta(minutes=adjusted_time_in_minutes)
    finishing_time_estimate = finishing_time.strftime('%H:%M:%S')
    ...
    if os.path.isfile(solverlap_file):
        ...
        print(f"{datetime.now().strftime('%H:%M:%S')} - Estimated runtime: {time_estimate} h, finishing at {finishing_time_estimate}.")
        # Compute similarity matrix
        soverlap_matrix = compute_similarity_matrix(seqs)
        # Save results
...
def phylogenetic_approach(output_directories, mafft_executable, vsearch_executable, similarity_threshold, cpu_count):
    ...
    # Get actual CPU usage efficiency
    cpu_utilization = psutil.cpu_percent(interval=1) / 100  # Get CPU usage over 1 second
    efficiency_factor = max(1, cpu_count * cpu_utilization)  # Avoid division by zero
    ...
    rerun_families = {family: run_soverlap(family, folder, similarity_threshold, efficiency_factor) for family in families}
