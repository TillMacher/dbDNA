...
def dereplicate_sequences(fasta_file, fasta_file_derep, fasta_file_pkl, family):
    ...
    counts = [len(ids) for ids in res.values()]
    max_seq = max(counts)
    avg_seq = round(np.mean(counts), 1)
    ...
    print(f'{datetime.now().strftime("%H:%M:%S")} - Dereplicated {family} (max={max_seq}, avg={avg_seq}).')
...
def run_mafft(family, mafft_executable, folder, cpu_count):
    ...
    if os.path.isfile(species_file_txt_snappy):
        message = f'{datetime.now().strftime("%H:%M:%S")} - {family} was already analyzed!'
    else:
        ...
        for record in raw_records:
            ...
            if any(c in special_characters for c in species):
                ambiguous_assignments.append([record[4], record[21], '', 'Ambiguous identification', '', -20]) # already write them in the final format for the rating
            else:
        ...
        if os.path.isfile(aln_file) and os.path.getsize(aln_file) > 0:
            message = f'{datetime.now().strftime("%H:%M:%S")} - Alignment was already calculated for {family}.'
        elif len(usable_records) < 4:
            message = f'{datetime.now().strftime("%H:%M:%S")} - Skipped alignment for {family} (less than 4 sequences).'
            ...
            message = f'{datetime.now().strftime("%H:%M:%S")} - Finished MAFFT alignment for {family}.'

    print(message)
    print('')

def run_trimal(family, trimal_executable, folder):
    """
    Function to run TrimAl for a specified family and handle sequence filtering.
    """

    #print(f'{datetime.now().strftime("%H:%M:%S")} - Starting TrimAl analysis for {family}.')
    ...
    aln_file = Path(f'{family_dir}/1_{family}_alignment.fasta')
    aln_file_adjusted = Path(f'{family_dir}/1_{family}_alignment_adjusted.fasta')
    aln_file_tmp = Path(f'{family_dir}/1_{family}_alignment_tmp.fasta')
    trimal_coverage_file = Path(f'{family_dir}/1_{family}_trimal_coverage.tsv')

    # Check if alignment already exists or is missing
    if os.path.isfile(aln_file_adjusted) and os.path.getsize(aln_file_adjusted) > 0:
        print(f'{datetime.now().strftime("%H:%M:%S")} - Alignment was already calculated for {family}.')
        return False
    elif not os.path.isfile(aln_file):
        print(f'{datetime.now().strftime("%H:%M:%S")} - No alignment was found for {family}.')
        return False
    elif os.path.isfile(trimal_coverage_file):
        print(f'{datetime.now().strftime("%H:%M:%S")} - TrimAL was already run for {family}.')
        return False

    # Run TrimAl command
    #print(f'{datetime.now().strftime("%H:%M:%S")} - Running TrimAl...')
    command = f"{trimal_executable} -in {aln_file} -out {aln_file_tmp} -automated1 -soverlap > {trimal_coverage_file}"
    process = subprocess.Popen(command, shell=True)
    process.wait()

    try:
        df = pd.read_csv(trimal_coverage_file, delimiter="\t", header=None, skiprows=7, index_col=0, low_memory=False)
        df.columns = list(df.index.values) + ['NA']
        df = df.drop(columns=['NA'])

        # Remove non-convertable values to 0
        df = df.apply(pd.to_numeric, errors='coerce').fillna(0)

        # Identify IDs to be removed based on average coverage < 0.7
        remove_ids = [i.strip() for i in list(df[df.mean(axis=1) < 0.7].index)]

    except Exception as e:
        print(f'{datetime.now().strftime("%H:%M:%S")} - Error processing TrimAl output: {family} {e}')
        return False

    # Check for spurious sequences
    if not remove_ids:
        print(f'{datetime.now().strftime("%H:%M:%S")} - No spurious sequences found for {family}.')
        return False
    else:
        print(f'{datetime.now().strftime("%H:%M:%S")} - Removing {len(remove_ids)} spurious sequences from {family}.')
        return True
...
def rerun_mafft(family, mafft_executable, folder, cpu_count, rerun):
    ...
    # File paths
    fasta_file_filtered = Path(f'{family_dir}/1_{family}_filtered.fasta')
    aln_file = Path(f'{family_dir}/1_{family}_alignment.fasta')
    aln_file_adjusted = Path(f'{family_dir}/1_{family}_alignment_adjusted.fasta')
    aln_file_tmp = Path(f'{family_dir}/1_{family}_alignment_tmp.fasta')

    if rerun and os.path.isfile(aln_file):
        # Filter the FASTA file
        print(f'{datetime.now().strftime("%H:%M:%S")} - Filtering sequences and cleaning alignment.')
        with open(aln_file_tmp, "r") as input_handle, open(fasta_file_filtered, "w") as output_handle:
            for record in SeqIO.parse(input_handle, "fasta"):
                if record.id not in remove_ids:  # Check if the record's ID is not in the exclude list
                    # Clean sequences: remove 'N' and '-' characters
...
def run_vsearch(family, vsearch_executable, folder, cpu_count):
    ...
    if not os.path.isfile(aln_file_adjusted):
    ...
    for id in dereplication_dict.keys():
        if id not in added_ids:
            ...
            cluster_df = pd.DataFrame(rows, columns=['Sequence ID', 'Species Name', 'No.'])
            for record in cluster_df.values.tolist():
                res.append(record + ['trimal-removed', '', 0])
...
## prepare species delimitation table
def run_create_family_table(family, folder):
    """
    Collect all relevant data
    """
    # family = 'Asellidae'

    print(f'{datetime.now().strftime("%H:%M:%S")} - Starting to prepare result table for {family}.')
    ...
    # File paths
    raw_barcodes_table = Path(f'{family_dir}/2_{family}_raw_barcodes.parquet.snappy')
    ambiguous_barcodes_table = Path(f'{family_dir}/2_{family}_ambiguous_taxa.parquet.snappy')
    usable_barcodes_table = Path(f'{family_dir}/2_{family}_usable_taxa.parquet.snappy')
    species_file_txt_snappy = Path(f'{family_dir}/3_{family}_species.parquet.snappy')

    df1 = pd.read_parquet(raw_barcodes_table)
    df2 = pd.read_parquet(ambiguous_barcodes_table)
    df3 = pd.read_parquet(usable_barcodes_table)
    df4 = pd.read_parquet(species_file_txt_snappy)

    print(f'{family}')
    print(f'  Raw barcodes: {len(df1)}')
    print(f'  Usable records {len(df2)} and ambiguous records {len(df3)} -> {len(df2) + len(df3)}')
    print(f'  Clustered barcodes {len(df4)}')

# update ambiguous tables (for manual testing)
def update_ambiguous_table(families, folder):
...
def phylogenetic_approach(output_directories, mafft_executable, vsearch_executable, cpu_count):
    ...
    # trimal analysis (NOT in parallel, can cause memory issues, because it requires LOTS of ram... may change in future versions)
    print('{} - Starting trimal analysis.'.format(datetime.now().strftime("%H:%M:%S")))
    rerun_families = {family: run_trimal(family, trimal_executable, folder) for family in families}
    print('{} - Finished trimal analysis.\n'.format(datetime.now().strftime("%H:%M:%S")))
...
def collect_image():
    ...
    async def check_and_download_image(record_id, download_folder="downloads"):
        url = f"https://portal.boldsystems.org/result?query=%22{record_id}%22"
        ...
        async with async_playwright() as p:
            ...
            # Navigate to the record's URL
            await page.goto(url)

            # Wait for images to load
            try:
                await page.wait_for_selector("#images-div img", timeout=10000)

                # Get the image URL
                image_element = await page.query_selector("#images-div img")
                image_url = await image_element.get_attribute("src")

                # Download the image
                image_path = os.path.join(download_folder, f"{record_id}.jpg")
                async with page.expect_download() as download_info:
                    await page.click(f'img[src="{image_url}"]')
                    download = await download_info.value
                    await download.save_as(image_path)

                print(f"Image downloaded for record ID: {record_id} -> {image_path}")
                return True
            except:
    ...
    # Example usage
    record_id = "FBAQU880-10"
    asyncio.run(check_and_download_image(record_id))
...
# filter_list =
def filter_database():
    db_file = pd.read_parquet('/Volumes/Coruscant/dbDNA/MZB_Germany_v1_BarCodeBank/3_BarCodeBank/MZB_Germany_v1.BarCodeBank.parquet.snappy').fillna('')
    family = 'Haliplidae'
    db_filtered = db_file.loc[db_file['family_name'] == family]
    db_filtered.to_excel(f'/Volumes/Coruscant/dbDNA/MZB_Germany_v1_BarCodeBank/3_BarCodeBank/MZB_Germany_v1.BarCodeBank_{family}.xlsx', index=False)
...
def validate_database(output_directories):
    ...
    final_processIDs = set(database_df['record_id'].values.tolist())
    print(len(final_processIDs))

    for family in families:
        file = output_directories[2].joinpath(family, f'2_{family}_raw_barcodes.parquet.snappy')
        raw_table = pd.read_parquet(file)
        processIDs = set(raw_table['record_id'].values.tolist())
        missing = len(processIDs - final_processIDs)
        len_table = len(raw_table)
        perc_missing = round(missing / len_table * 100, 2)
        if missing != 0:
            print(f'Missing records: {family} ({perc_missing} %)')
...
def report(output_directories, project_name, taxa_list):
    ...
    taxa = sorted(reference_db_df[test_taxon].drop_duplicates().values.tolist())
    phylo_states = ['monophyletic', 'monophyletic (singleton)', 'polyphyletic', 'polyphyletic (LRT failed)', 'Ambiguous identification', 'Error']
...
## run main script
    ...
    project_name = variables.loc[variables['Variable'] == 'project name']['User input'].values.tolist()[0]
    ...
    min_rating = variables.loc[variables['Variable'] == 'rating minimum']['User input'].values.tolist()[0]
    ...
    trimal_executable = variables.loc[variables['Variable'] == 'trimal executable']['User input'].values.tolist()[0]
    ...
    if run_phylogeny == 'yes':
        phylogenetic_approach(output_directories, mafft_executable, vsearch_executable, cpu_count)
