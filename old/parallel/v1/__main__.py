...
def run_soverlap(family, folder, similarity_threshold, cpu_count):
    ...
    # File paths
    aln_file = Path(f'{family_dir}/1_{family}_alignment.fasta')
    solverlap_file = Path(f'{family_dir}/1_{family}_soverlap.csv')
    fasta_file_derep = Path(f'{family_dir}/1_{family}_derep.fasta')
    ...
    if os.path.isfile(solverlap_file):
        print(f'{datetime.now().strftime("%H:%M:%S")} - Overlap matrix was already run for {family}.')
        print(f'{datetime.now().strftime("%H:%M:%S")} - Reading existing overlap matrix for {family}.')

        overlap_df = pd.read_csv(solverlap_file, sep='\t', index_col=0)
        ids = overlap_df.index.tolist()
        soverlap_matrix = overlap_df.values

        if len(soverlap_matrix) != 0:
            # Calculate row means and check for IDs with low similarity
            ...
            if not remove_ids:
                ...
                print(f'{datetime.now().strftime("%H:%M:%S")} - Found spurious sequences in the existing overlap matrix for {family}.\n')
                return True
        else:
            print(f'{datetime.now().strftime("%H:%M:%S")} - Existing overlap matrix for {family} is empty. Recalculating overlap matrix.')

    # Collect all reads
    aln_dict = {str(record.id): str(record.seq).replace('-', 'N') for record in SeqIO.parse(aln_file, "fasta")}
    ids = list(aln_dict.keys())
    n = len(ids)
    ...
    overlap_df = pd.DataFrame(soverlap_matrix, columns=ids, index=ids)
    overlap_df.to_csv(solverlap_file, sep='\t')

    # Identify IDs to be removed based on average coverage < 0.5
    row_means = soverlap_matrix.mean(axis=1)
    remove_ids = [ids[i] for i, mean in enumerate(row_means) if mean < similarity_threshold]
    ...
    if not remove_ids:
        ...
        print(f'{datetime.now().strftime("%H:%M:%S")} - Removed {len(remove_ids)} spurious sequence(s) with < {similarity_threshold} avg. similarity from {family}.\n')
