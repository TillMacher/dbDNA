...
def run_soverlap(family, folder, similarity_threshold, cpu_count):
    ...
    if os.path.isfile(solverlap_file):
        ...
        results = Parallel(n_jobs=cpu_count, backend='threading')(delayed(compute_row_partial)(i, aln_dict, ids) for i in tqdm(range(n), desc=f'{datetime.now().strftime("%H:%M:%S")} - Processing sequences'))

        # Construct the full symmetric matrix
        soverlap_matrix = np.zeros((n, n), dtype=np.float32)
        i = 0
        for row in tqdm(results, desc=f'{datetime.now().strftime("%H:%M:%S")} - Constructing overlap matrix'):
